...
export function EditorSidebar({ elements }) {
  ...
  return (
    <section className="editor-sidebar">
          ...
          <div ref={provided.innerRef} {...provided.droppableProps} className="cmps-list">
            {elements &&
              elements.map((elements, idx) => {
                return (
                  <Draggable key={elements.id} draggableId={elements.id} index={idx}>
                    {(provided, snapshot) => {
                      return (
                        <>
                          <img ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} key={elements.id} className="mini-cmp" src={elements.thumbnail} alt="" />
                          {snapshot.draggingOver ? <img key={elements.id} className="mini-cmp copy-dnd" src={elements.thumbnail} alt="" /> : null}
                        </>
                      )
                    }}
                  </Draggable>
      ...
      >
        <svg width={'15'} height="96" viewBox="0 0 15 96" className="sidebar-close-btn">
          <path
