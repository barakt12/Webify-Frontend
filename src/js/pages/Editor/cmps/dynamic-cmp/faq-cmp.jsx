...
const Accordion = styled((props) => <MuiAccordion sx={{ outline: 'none !important' }} disableGutters elevation={0} square {...props} />)(({ theme }) => ({
...
const AccordionSummary = styled((props) => (
  <MuiAccordionSummary sx={{ outline: 'none !important' }} expandIcon={<ArrowForwardIosSharpIcon sx={{ fontSize: '0.9rem', outline: 'none !important' }} />} {...props} />
))(({ theme }) => ({
...
export function FAQCmp({ cmp, onHoverCmp, selectedCmp, onSelectCmp, displayClass }) {
      ...
      {cmp.info.questions.map((question, idx) => {
        return (
          <Accordion sx={{ backgroundColor: 'transparent' }} key={idx} expanded={expanded === `panel${idx + 1}`} onChange={handleChange(`panel${idx + 1}`)}>
            <AccordionSummary sx={{ outline: 'none !important' }} aria-controls={`panel${idx + 1}d-content`} id={`panel${idx + 1}d-header`}>
